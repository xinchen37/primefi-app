...
- Optional `displayDecimals` on each token controls quantity display, defaults to 2, and accepts integers 0–100. For example, WETH can use `decimals: 18, displayDecimals: 6`. It does not affect ERC-20 decimals, amount parsing, MAX, approvals, or transaction calldata. Amounts are floored and padded to the configured number of places; full available values remain in tooltips.
