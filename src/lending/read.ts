...
export interface AssetSnapshot {
  ...
  price: bigint; supplyApy: number; borrowApy: number; ltv: number; active: boolean; frozen: boolean; paused: boolean; borrowing: boolean; collateral?: boolean;
}
export interface PoolSnapshot { assets: AssetSnapshot[]; collateral: bigint; debt: bigint; available: bigint; health: bigint; unit: bigint }
export async function readPool(io: Pick<LendingIO, 'read'>, market: LendingMarket, account?: Address): Promise<PoolSnapshot> {
  ...
  return { assets, collateral: data[0], debt: data[1], available: data[2], health: data[5], unit };
}
