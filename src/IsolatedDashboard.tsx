  ...
  return <>
    ...
    <div className="position-grid isolated-position-grid">
      ...
      <section className="panel"><div className="panel-title"><h3>Your borrows</h3></div>
        ...
        {borrows.length ? <div className="table-wrap"><table><thead><tr><th>Borrow / collateral</th><th>Debt / APY</th><th>Health factor</th><th /></tr></thead><tbody>{borrows.map(({ m, p, t }) => <tr key={m.symbol}><td>{identity(m, true)}</td><td><strong>{number(p.debt)} USDG</strong><small>{m.borrowApy.toFixed(2)}% APY</small></td><td><Health value={t.hf} /></td><td><button className="secondary" disabled={isolatedMaximum('repay', m, p, usdgWallet) <= 0} onClick={() => action(m, 'repay')}>Repay</button></td></tr>)}</tbody></table></div> : empty('Supply isolated collateral to start borrowing USDG.')}
    ...
    <div className="position-grid asset-panels">
      <section className="panel"><div className="panel-title"><h3>Assets to supply</h3><span className="subtle">Available balance</span></div>
        <div className="table-wrap"><table><thead><tr><th>Asset</th><th>Wallet balance</th><th>Max LTV</th><th /></tr></thead><tbody>{rows.map(({ m, p }) => <tr key={m.symbol}><td>{identity(m)}</td><td><strong>{connected ? number(p.wallet) : '—'}</strong><small>{connected ? money(p.wallet * m.price) : 'Connect to view'}</small></td><td>{(m.ltv * 100).toFixed(0)}%</td><td>{availableAction(m, 'supply', connected && p.wallet <= 0)}</td></tr>)}</tbody></table></div>
      ...
      <section className="panel"><div className="panel-title"><h3>Assets to borrow</h3><span className="subtle">USDG only</span></div>
        <div className="table-wrap"><table><thead><tr><th>Borrow / collateral</th><th>Available</th><th>Borrow APY</th><th /></tr></thead><tbody>{rows.map(({ m, p }) => { const max = connected ? isolatedMaximum('borrow', m, p, usdgWallet) : 0; return <tr key={m.symbol}><td>{identity(m, true)}</td><td><strong>{connected ? number(max) : '—'}</strong><small>{connected ? 'USDG' : 'Connect to view'}</small></td><td>{m.borrowApy.toFixed(2)}%</td><td>{availableAction(m, 'borrow', connected && max <= 0)}</td></tr>; })}</tbody></table></div>
