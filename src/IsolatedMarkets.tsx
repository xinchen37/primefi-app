  ...
  return <>
    ...
    <section className="panel table-wrap isolated-table"><table><thead><tr><th>Collateral → Borrow</th><th>Total collateral</th><th>Max LTV</th><th>Borrow APY</th><th>Debt ceiling used</th><th>Available to borrow</th><th /></tr></thead><tbody>
      {markets.map(m => { const t = isolatedTotals(m, portfolio[m.symbol]); return <tr key={m.symbol}>
        ...
        <td><strong>${compact(t.totalCollateral * m.price)}</strong><small>{compact(t.totalCollateral)} {m.symbol}</small></td><td>{(m.ltv * 100).toFixed(0)}%</td><td>{m.borrowApy.toFixed(2)}%</td>
        <td><strong>${compact(t.totalDebt)} / ${compact(m.debtCeiling)}</strong><div className="progress"><i style={{ width: `${Math.min(100, t.totalDebt / m.debtCeiling * 100)}%` }} /></div><small>{(t.totalDebt / m.debtCeiling * 100).toFixed(1)}% used</small></td><td>${compact(Math.min(t.remaining, t.liquidity))} USDG</td>
