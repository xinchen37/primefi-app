...
export default function RateChart({ rate, label, borrow = false }: { rate: number; label: string; borrow?: boolean }) {
  ...
  return <section className={`rate-chart ${borrow ? 'borrow-chart' : ''}`} aria-label={label}>
    ...
    <div className="rate-readout" aria-live="polite">Average {average.toFixed(2)}%</div>
    <div className="rate-chart-canvas">
      <ResponsiveContainer width="100%" height={220} minWidth={0}>
        <LineChart data={points} accessibilityLayer margin={{ top: 18, right: 18, bottom: 8, left: 0 }} aria-label={`${label}, ${period}, average ${average.toFixed(2)} percent`}>
          ...
          <Tooltip labelFormatter={value => new Date(Number(value)).toLocaleString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' })} formatter={value => [`${Number(value).toFixed(2)}%`, label]} cursor={{ stroke: '#b8b1c4', strokeDasharray: '4 4' }} contentStyle={{ border: '1px solid #e6e3ed', borderRadius: 8, fontSize: 14, boxShadow: '0 4px 16px #29273512' }} labelStyle={{ color: '#777381', fontSize: 12 }} isAnimationActive={false} />
